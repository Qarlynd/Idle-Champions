...
global gFkeySpam := 0
global gRight := 1
global gJimStopZone := 1495
global gSpeedTime := 0
global gBrivZone := 500
global gAzakaFarm := 0
global gSpamQ := 1
global gMaxMonsters := 125
global gMaxLevel := 1495
...
Jimothy_Clicked:
{
	...
	loop
	{
		...
		if (ReadCurrentZone(1) > 2000)
		{
			StopProgression()
			GuiControl, MyWindow:, Jimothy_ClickedID, No
			Break
		}
        ;sleep, ScriptSpeed
	}
...
SwapOutBriv()
{
	...
	j := mod(i, 5)
	;for 7/8x skip and to avoid z7 and z28 on Briv TG make sure to start jumping on zone 3, below should get you back to right sequence if started early enough
	;if (j = 1 OR j = 2 OR i = 8 OR i = 9 OR i = 10)
	;for 7/8x skip and no special levels to avoid
	;if (j = 1 OR j = 2)

	;for 7 skip and to avoid z7 and z28 on Briv TG
	;if (j = 2 OR j = 1 OR i = 9 OR i = 10)
	;for 7 skip and no special levels to avoid
	;if (j = 2)
	if (j = 1)
	{
		DirectedInput("e")
